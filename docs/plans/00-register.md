...
| RR8 | [engine-writes-unreadable-state] The engine can write a session its own validator refuses to read, and the resume shelf offers it anyway. `setTeams` stranding strikes was one instance (`M19`); `addStrike` lacking a COLUMN bound was a second, found the same day by an assertion that revalidated after writing. The class is what matters: `update()`'s `cheapCheck` does not run the validator, so any mutator can produce an unloadable document, and `listSessions`/`summaryOf` read only the title and timestamp, so the row is still offered and dies on click with Discard as the only exit. | ratified | — | — | Either revalidate on write in a debug build, or have `listSessions` mark a row that will not load so the host is not offered a dead game mid-show | agent | Evidence: two instances in one feature; both reproduced end-to-end in the real UI and both now carry a mutation |
