...
| M13 | `js/state.js` | `currentRound` is held in view state instead of the session, so a mid-show reload returns to Round 1 with the scores intact — the failure a host would notice only on stage. | a resume assertion that reloads a board parked on Round 3 | open — planned for `F9c-T5` | `D17` |
| M14 | `js/renderer.js` | Non-active rounds are hidden with CSS alone, so their cells stay in the accessibility tree and reachable by `Tab` — a keyboard host can open a cell in a round the room cannot see. | a focus assertion, the same class as the overlay's `inert` fix in `openCell` | open — planned for `F9c-T5` | `D17` |
