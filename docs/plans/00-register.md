...
| M9 | `js/schemas.js` | An imported `"strikes": {"0": 99}` is accepted unvalidated and renders 99 marks. | `stateStrikesInBounds` in the validator | closed — caught while UNPLANNED: the suite caught my own test writing feud strikes into a live jeopardy session, and the round-trip assertion failed on it. The check works; the test was wrong | `D15` |
...
| M12 | `themes/*.css` | The marks are styled only in `default.css`, so a themed board shows nothing. | `a strike mark is drawn large enough to read from the back of a room`, measured inside the booted iframe | closed — **SURVIVED the first time**, 404/404 green while a mark rendered 4x4px, exactly the hole `M5` found: every render assertion draws into `harnessStage()`, which carries no stylesheet. With the shell assertion added, M12 fails 404/405 | `D16` |
