...
| M24 | `editor/editor.js` | The bytes downloaded are not the bytes judged — validate the object, serialise separately. | `the exported text is the thing that was validated, and survives a round trip`, which re-judges the STRING | closed — prevented by construction: `toFileText()` is called once and its output is both judged and handed to the Blob |
